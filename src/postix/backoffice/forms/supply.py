...
class SupplyCreateForm(forms.Form):
    ...
    def clean_identifier(self):
        ...
        if ItemSupplyPack.objects.filter(identifier=self.cleaned_data['identifier']).exists():
...
class SupplyMoveForm(forms.Form):
    ...
    def clean_identifier(self):
        ...
        if self.require_state and isp.state != self.require_state:
            raise ValidationError('This pack is currently in state "{}", no action performed!'.format(isp.get_state_display()))
        return isp
