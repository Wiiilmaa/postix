...
class AssetMoveView(BackofficeUserRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if not location:
            if not position:
                messages.error(self.request, _('Asset was not marked as away, does not need to be returned'))
                return self.form_invalid(form)
            position.end = now()
            ...
            if position:
                ...
                position.save()
                asset.save()
            AssetPosition.objects.create(asset=asset, start=now(), location=location, comment=comment)
            return redirect(self.get_success_url())
