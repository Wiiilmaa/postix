...
class SupplyCreateView(BackofficeUserRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        isp.logs.create(
            ...
            new_state='backoffice'
        )
        messages.success(self.request, _('The supply pack {id} has been created.').format(id=form.cleaned_data['identifier']))
        return redirect(self.request.path + '?amount={amount}&item={item.pk}'.format_map(form.cleaned_data))
...
class SupplyMoveOutView(BackofficeUserRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        form.cleaned_data['identifier'].logs.create(
            user=self.request.user,
            new_state='troubleshooter'
        )
        messages.success(self.request, _('The supply pack {id} has been moved to the troubleshooter.').format(id=form.cleaned_data['identifier']))
...
class SupplyMoveInView(BackofficeUserRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, _('The supply pack {id} has been moved to the backoffice.').format(id=form.cleaned_data['identifier']))
        return redirect(self.request.path)
...
class SupplyMoveAwayView(BackofficeUserRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        form.cleaned_data['identifier'].save()
        messages.success(self.request, _('The supply pack {id} has been dissolved.').format(id=form.cleaned_data['identifier']))
        return redirect(self.request.path)
